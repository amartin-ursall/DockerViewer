...
import { Server, Trash2, Edit, Zap, GripVertical, Star } from 'lucide-react';
...
interface ConnectionItemProps {
  connection: Connection & { isDefault?: boolean };
...
const ConnectionItem = ({ connection, onConnect, onDelete, onEdit, onSetDefault, isConnecting, isDraggable }: ConnectionItemProps) => {
  ...
  return (
    <div ref={setNodeRef} style={style} className="p-3 rounded-lg bg-gray-900/50 border border-gray-700/50 hover:border-teal-500/50 transition-all duration-200 flex flex-col gap-2">
      <div className="flex items-center gap-2">
        ...
        <div className="flex-grow">
          <p className="font-semibold text-white flex items-center gap-2">
            {connection.alias}
            {connection.isDefault && <Star className="w-4 h-4 text-yellow-400 fill-yellow-400" />}
          </p>
      ...
      <div className="flex items-center justify-end space-x-2 pl-2">
        <Label htmlFor={`default-switch-${connection.id}`} className="text-xs text-gray-400">Set as Default</Label>
        <Switch
          id={`default-switch-${connection.id}`}
          checked={connection.isDefault}
          onCheckedChange={(checked) => onSetDefault(connection.id, checked)}
        />
      </div>
    </div>
  );
...
interface ConnectionListProps {
  connections: (Connection & { isDefault?: boolean })[];
  onConnect: (connection: Connection) => void;
  onDelete: (id: string) => void;
  onEdit: (connection: Connection) => void;
  onSetDefault: (id: string, isDefault: boolean) => void;
...
export function ConnectionList({ connections, onConnect, onDelete, onEdit, onSetDefault, isConnecting, children }: ConnectionListProps) {
  const content = (
